...
def test_vllm_config_rejects_sharded_cp_with_sequence_parallelism():
    """SP pads scheduled tokens to a TP multiple, which breaks CP rows."""
...
def test_vllm_config_rejects_sharded_cp_with_torch_compile():
    """cudagraph_mode NONE alone must not let a compiled graph through: the
    CP path issues raw collectives Dynamo cannot trace."""
