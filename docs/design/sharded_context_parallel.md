...
The in-tree implementation follows this design with the following deltas,
chosen after reviewing the original research branch:
...
- **Complete caches on every rank.** Every CP step all-gathers the compact
  `[kv_c || k_pe || indexer_k]` payload and writes all global rows into the
...
- **Fail-closed scope.** v1 requires `--enforce-eager` (Shard Linear swaps
  ...
  decoding, no model-weight offloading, and the `FLASHMLA_SPARSE` backend
  (auto-selected backends are re-validated in the model runner). Batches
  whose backend metadata does not support global-compact-KV overrides
  automatically fall back to the paged-KV path.
- **Shared top-k (IndexCache) layers stay on the CP path.** Layers built
  with `indexer=None` (`index_topk_freq > 1` or an `"S"` pattern entry)
  still all-gather the MLA compact KV and persist the full batch into their
  caches; they only skip the Indexer-K payload and top-k scoring, reusing
  the previous layer's global top-k buffer.
- **Dedicated prefetch communicator.** Shard Linear broadcasts run on their
  own process group and CUDA stream so the per-layer compact-KV all-gather
  is not queued behind future layers' weight broadcasts.
- **Known cost: `kv_b_proj` is full-size per rank.** Sharded-CP needs all
  heads locally, so `kv_b_proj` uses `disable_tp=True` and is *not* managed
  by Shard Linear: it is still used at runtime for the chunked-prefill
  context and `W_UK`/`W_UV` are derived from it at load time. Both the
  matrix and the absorbed copies therefore stay resident at full size on
  every rank instead of being TP-sharded, which for a 61-layer DeepSeek
  V3.2 adds multiple GB per rank. Bringing it under owner broadcast
  requires making the absorbed weights materializable too.
- **Known gap: MoE gathers BF16 activations.** The design calls for
  gathering quantized activations, scales, and routing metadata; the current
...
- **Indexer fused GEMM preserved.** The CP path splits the fused
  `wk_weights_proj` output instead of splitting the projection into two
  GEMMs, so the flag-off path is untouched.
