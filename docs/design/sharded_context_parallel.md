...
> **Superseded.** This section records the thesis design. The in-tree
> implementation does *not* use Shard Linear for `o_proj`: an all-to-all
> converts the attention output into the tensor-parallel head layout so
> `o_proj` keeps its normal TP-sharded weight. See
> "All-to-all instead of Shard Linear" under Implementation Notes below.
...
### Commit 6: Synchronous Shard Linear (superseded by the all-to-all)
...
### Commit 7: Async Broadcast / Prefetch (superseded by the all-to-all)
...
```bash
# CPU: token-range partitioning + config validation
.venv/bin/python -m pytest tests/distributed/test_sharded_cp_utils.py -v
.venv/bin/python -m pytest tests/config/test_sharded_context_parallel.py -v
# Multi-GPU collectives, and end-to-end generation parity:
bash tools/sharded_cp/run_sharded_cp_tests.sh dist
MODEL_PATH=/path/to/DeepSeek-V3.2 bash tools/sharded_cp/run_sharded_cp_tests.sh e2e
```
...
## Implementation Notes for the v0.26 Port

The in-tree implementation follows this design with the deltas below. Notably
the Shard Linear scheme from the thesis is **replaced by an all-to-all head
layout exchange**, following the approach used by vllm-ascend's DSA-CP
(`vllm_ascend/attention/context_parallel/dsa_cp.py`).

### All-to-all instead of Shard Linear

Attention under Sharded-CP computes **all heads for this rank's token rows**,
so naively both `q_b_proj` and `o_proj` need full logical weights. The thesis
solves that by keeping one layer's full weights on an owner rank and
broadcasting them on demand (Shard Linear). That is not needed for `o_proj`:

```text
attention out:  [T_local, H_full,  v_head_dim]     # CP rows, all heads
   all-to-all:  [T_global, H_local, v_head_dim]     # all rows, TP heads
      o_proj:   standard TP-sharded weight, reduce_results=False
reduce-scatter: [T_local, hidden]                   # TP reduction + back to CP
```

One `all_to_all_single` converts the CP layout into exactly the layout a
tensor-parallel `RowParallelLinear` expects, so `o_proj` keeps its normal
TP-sharded weight. Its usual all-reduce is replaced by a reduce-scatter over
token rows, which performs the same TP reduction *and* returns to CP-local
rows in one collective. See
`vllm/distributed/sharded_cp_utils.py::all_to_all_cp_rows_to_tp_heads` and
`vllm/model_executor/layers/mla.py::_sharded_cp_o_proj`.

Consequences, versus the Shard Linear version:

- `o_proj` — by far the largest projection (~112 MiB/layer INT8 for DeepSeek
  V3.2, against ~24 MiB for `q_up`) — is TP-sharded again instead of being
  replicated or owner-broadcast.
- No owner-broadcast machinery at all: no per-layer weight broadcast, no K=2
  prefetch, no dedicated communicator, no released/materialized parameter
  storage. Parameter shapes are static for the whole run, which removes the
  `torch.compile` incompatibility, the `reload_weights` shape mismatch, and
  the weight-offload conflict that the Shard Linear version had to fail
  closed on.
- The collective moves activations, not weights: `T_global x H_local x
  v_head_dim` per layer. For long prefills that can exceed the cost of moving
  a fixed-size weight, which is why vllm-ascend keeps an opt-in variant that
  all-gathers the `o_proj` weight for prefill and uses the all-to-all only
  for decode. Not implemented here.

`q_b_proj` (and `kv_b_proj`, whose absorbed `W_UK`/`W_UV` must cover all
heads) still use full logical weights via `disable_tp=True`, replicated on
every rank. Removing that would need a second all-to-all on `q`, or the
weight-gather variant above.

### Other deltas

- **Metadata via builders, not hand slicing.** The runner localizes
  `CommonAttentionMetadata` to the rank's token range (reusing the ubatch
  slicing helpers) and re-runs the registered attention metadata builders on
  the localized view (`vllm/v1/attention/backends/mla/sharded_cp_metadata.py`).
  Backend metadata therefore inherits chunking, decode expansion, and future
  builder changes for free. Only the global-compact-KV override remains
  backend specific.
- **The runner owns the CP forward context.** `_build_attention_metadata`
  produces the CP run state and `execute_model`/`_dummy_run` wrap the model
  call, so the model only consumes `additional_kwargs`.
- **Compact KV instead of gathering hidden states.** vllm-ascend all-gathers
  the layer's hidden states (7168 wide) and recomputes KV redundantly on
  every rank. This implementation instead all-gathers the compact
  `[kv_c || k_pe || indexer_k]` payload (704 wide) and writes all global rows
  into the local MLA and indexer caches with global slot IDs, including on
  ranks that own zero rows. Cheaper on the wire; caches stay complete when
  requests move between ranks across steps. This is still the
  correctness-stage layout: KV memory savings from CP-aware cache ownership
  are left to a follow-up.
- **Explicit scatter/gather, no sequence parallelism dependency.**
  vllm-ascend's DSA-CP requires FlashComm1 sequence parallelism, which gives
  it CP-sharded rows for free from each layer's row-parallel reduce-scatter.
  Upstream vLLM has no equivalent, so the layout boundaries are explicit:
  reduce-scatter after the (vocab-parallel) embedding, all-gather before the
  LMHead, and gather/reduce-scatter around MoE.
- **Shared top-k (IndexCache) layers stay on the CP path.** Layers built with
  `indexer=None` (`index_topk_freq > 1` or an `"S"` pattern entry) still
  all-gather the MLA compact KV and persist the full batch into their caches;
  they only skip the Indexer-K payload and top-k scoring, reusing the
  previous layer's global top-k buffer.
- **Zero-token ranks are handled throughout:** RMSNorm, SiluAndMul, the
...
- **Indexer fused GEMM preserved.** The CP path splits the fused
  `wk_weights_proj` output instead of splitting the projection into two GEMMs,
  so the flag-off path is untouched.
