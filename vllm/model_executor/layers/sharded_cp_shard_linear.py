...
layer. Other ranks materialize those parameters before the layer consumes
them, either synchronously or by waiting on a previously issued async
prefetch.
"""
...
_PREFETCH_PG: dist.ProcessGroup | None = None
_PREFETCH_STREAMS: dict[torch.device, torch.cuda.Stream] = {}


def ensure_sharded_cp_prefetch_group(group=None) -> dist.ProcessGroup | None:
    """Create (once) a dedicated communicator for Shard Linear broadcasts.

    Weight prefetch broadcasts must not share a communicator with the
    per-layer compact-KV all-gathers: NCCL executes collectives of one
    communicator in issue order, so a gather queued behind the next layers'
    large weight broadcasts would lose all overlap with compute.

    Must be called collectively, in the same order on every rank (e.g. from
    model construction).
    """
    global _PREFETCH_PG
    if _PREFETCH_PG is None and dist.is_available() and dist.is_initialized():
        resolved = group if group is not None else get_sharded_cp_group()
        if hasattr(resolved, "ranks"):
            _PREFETCH_PG = dist.new_group(list(resolved.ranks))
    return _PREFETCH_PG


def _get_prefetch_stream(device: torch.device) -> torch.cuda.Stream:
    stream = _PREFETCH_STREAMS.get(device)
    if stream is None:
        stream = torch.cuda.Stream(device=device)
        _PREFETCH_STREAMS[device] = stream
    return stream


def destroy_sharded_cp_prefetch_group() -> None:
    """Release the prefetch communicator and its streams.

    Hooked into ``destroy_model_parallel()`` so an in-process engine
    re-initialization does not broadcast on a communicator whose backing
    distributed environment is gone.
    """
    global _PREFETCH_PG
    if _PREFETCH_PG is not None:
        if dist.is_available() and dist.is_initialized():
            dist.destroy_process_group(_PREFETCH_PG)
        _PREFETCH_PG = None
    _PREFETCH_STREAMS.clear()
...
class ShardedCPShardLinearPrefetch:
    ...
    def __init__(
        ...
        events: list[tuple[torch.cuda.Event, torch.device]],
    ) -> None:
        self._layer = layer
        self._group = group
    ...
    def wait(self) -> None:
        ...
        for handle in self._handles:
            if handle is not None:
                handle.wait()
        # Event-based device-side ordering: wait only for this layer's
        # broadcasts, not for later work enqueued on the shared comm stream.
        for event, device in self._events:
...
class ShardedCPShardLinearLayer:
    ...
    def describe_parameters(self, group=None) -> list[ShardedCPShardLinearParam]:
        ...
        owner_rank = get_sharded_cp_shard_linear_owner(self.layer_id, group.world_size)
    ...
    def restore_full_storage(self) -> bool:
        """Reallocate released parameters so a weight loader can fill them.

        Returns True if anything was restored, i.e. this is a reload of an
        already-released model. Idempotent on a freshly built model.
        """
        restored = False
    ...
    def _broadcast_async(
        ...
        process_group = ensure_sharded_cp_prefetch_group(group) or group.device_group
        if stream is None:
    ...
    ) -> tuple[list[dist.Work | None], list[tuple[torch.cuda.Event, torch.device]]]:
        if group.world_size == 1:
            return [], []
                ...
                if async_broadcast:
                    ...
                    if param.data.is_cuda:
                        stream = stream_by_device.setdefault(
                            param.data.device,
                            _get_prefetch_stream(param.data.device),
                        )
                    handles.append(
                        self._broadcast_async(param.data, owner_rank, group, stream)
                    )
                    if stream is not None:
                        event = torch.cuda.Event()
                        event.record(stream)
                        events.append((event, param.data.device))
                else:
    ...
    def prefetch(self, group=None) -> ShardedCPShardLinearPrefetch:
        ...
        group = group if group is not None else get_sharded_cp_group()
        handles, events = self._materialize_params(group, async_broadcast=True)
        return ShardedCPShardLinearPrefetch(self, group, handles, events)
