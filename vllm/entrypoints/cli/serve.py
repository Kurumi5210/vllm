...
def run_multi_api_server(args: argparse.Namespace):
    ...
    with (
        launch_domain_core_engines(vllm_config, executor_class, log_stats, num_api_servers) 
        if vllm_config.parallel_config.dp_per_domain > 1 
        else launch_core_engines(vllm_config, executor_class, log_stats, num_api_servers)
