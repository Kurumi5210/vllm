...
class GPUModelRunner(
    ...
    def reload_weights(
        ...
        # Sharded-CP releases non-owner Shard Linear weights to shape [0]
        # after loading. Re-materialize them before the reload machinery
        # snapshots kernel tensors, otherwise the processed full weights are
        # copied back into the [0] tensors and fail on shape mismatch.
        restore_sharded_cp = getattr(model, "restore_sharded_cp_full_weights", None)
        release_sharded_cp = getattr(model, "post_process_weights_after_loading", None)
        if restore_sharded_cp is not None:
            restore_sharded_cp()
        ...
        if release_sharded_cp is not None:
            release_sharded_cp()
