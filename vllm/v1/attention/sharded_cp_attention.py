...
"""Sharded-CP attention helpers.

These helpers keep the compact KV communication payload explicit and
unit-testable before the sparse MLA kernel path is enabled end to end.
"""
...
def _flatten_k_pe(k_pe: torch.Tensor) -> torch.Tensor:
    ...
    raise ValueError(
        "k_pe must have shape [num_tokens, rope_dim] or "
...
def pack_sharded_cp_compact_kv(
    ...
    if not (
        kv_c_normed.shape[0] == k_pe_flat.shape[0] == indexer_k.shape[0]
    ):
...
def split_sharded_cp_compact_kv(
    ...
    _validate_positive_dim("indexer_head_dim", layout.indexer_head_dim)
    if compact_kv.dim() != 2:
...
def assemble_sharded_cp_compact_kv_chunks(
    gathered: list[torch.Tensor],
    token_range: ShardedCPTokenRange,
    layout: ShardedCPCompactKVLayout,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Assemble padded compact KV chunks and split the global payload."""
    compact_kv = assemble_token_all_gather_chunks(gathered, token_range)
    return split_sharded_cp_compact_kv(compact_kv, layout)
